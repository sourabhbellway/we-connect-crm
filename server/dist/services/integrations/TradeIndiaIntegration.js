...
exports.TradeIndiaIntegration = void 0;
const axios_1 = __importDefault(require("axios"));
const BaseIntegration_1 = require("./BaseIntegration");
class TradeIndiaIntegration extends BaseIntegration_1.BaseIntegration {
    constructor(prisma, config, integrationId) {
        super(prisma, config, integrationId);
        this.baseUrl = 'https://api.tradeindia.com/api';
    }
    getName() {
        return 'TradeIndia';
    }
    ...
    async testConnection() {
        try {
            const response = await axios_1.default.get(`${this.baseUrl}/leads`, {
                params: {
    ...
    async fetchLeads(lastSyncDate, pageToken) {
        try {
            ...
            const response = await axios_1.default.get(`${this.baseUrl}/leads`, { params });
            if (response.data.status !== 'success') {
                await this.logOperation('fetch_leads', 'FAILED', `TradeIndia API returned error: ${response.data.message || 'Unknown error'}`, response.data, response.data.message);
                return {
                    success: false,
            ...
            await this.logOperation('fetch_leads', 'SUCCESS', `Successfully fetched ${leads.length} leads from TradeIndia`, {
                totalCount: leads.length,
                pagination: response.data.pagination,
            }, undefined, leads.length);
            return {
                success: true,
                data: leads,
            ...
            await this.logOperation('fetch_leads', 'FAILED', 'Error fetching leads from TradeIndia', undefined, errorMessage);
            return {
                success: false,
                error: errorMessage,
                data: [],
...
exports.TradeIndiaIntegration = TradeIndiaIntegration;
//# sourceMappingURL=TradeIndiaIntegration.js.map