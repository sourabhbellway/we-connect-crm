...
exports.IndiaMartIntegration = void 0;
const axios_1 = __importDefault(require("axios"));
const BaseIntegration_1 = require("./BaseIntegration");
class IndiaMartIntegration extends BaseIntegration_1.BaseIntegration {
    constructor(prisma, config, integrationId) {
        super(prisma, config, integrationId);
        this.baseUrl = 'https://mapi.indiamart.com/wservce/crm/crmListing/v2';
    }
    getName() {
        return 'IndiaMart';
    }
    ...
    async fetchLeads(lastSyncDate, pageToken) {
        try {
            ...
            if (response.data.status !== 'SUCCESS') {
                await this.logOperation('fetch_leads', 'FAILED', `IndiaMart API returned error: ${response.data.message || 'Unknown error'}`, response.data, response.data.message);
                return {
                    success: false,
            ...
            await this.logOperation('fetch_leads', 'SUCCESS', `Successfully fetched ${leads.length} leads from IndiaMart`, { totalCount: leads.length }, undefined, leads.length);
            return {
                success: true,
                data: leads,
            ...
            await this.logOperation('fetch_leads', 'FAILED', 'Error fetching leads from IndiaMart', undefined, errorMessage);
            return {
                success: false,
                error: errorMessage,
                data: [],
...
exports.IndiaMartIntegration = IndiaMartIntegration;
//# sourceMappingURL=IndiaMartIntegration.js.map