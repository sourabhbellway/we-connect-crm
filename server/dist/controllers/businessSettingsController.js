...
exports.businessSettingsController = exports.BusinessSettingsController = void 0;
const express_validator_1 = require("express-validator");
const prisma_1 = require("../lib/prisma");
const IntegrationManager_1 = require("../services/integrations/IntegrationManager");
const IntegrationScheduler_1 = require("../services/IntegrationScheduler");
// Helpers to map DB BusinessSettings -> frontend CompanySettings shape
function mapDBToCompanySettings(settings) {
    return {
...
class BusinessSettingsController {
    ...
    async getAllBusinessSettings(req, res) {
        try {
    ...
    async updateCompanySettings(req, res) {
        try {
            const { name, email, phone, address, website, logo, timezone, gstNumber, panNumber, cinNumber, fiscalYearStart, industry, employeeCount, description, } = req.body || {};
            const updateData = {};
            if (name !== undefined)
                updateData.companyName = name;
            if (email !== undefined)
                updateData.companyEmail = email;
            if (phone !== undefined)
                updateData.companyPhone = phone;
            if (address !== undefined)
                updateData.companyAddress = address;
            ...
            if (cinNumber !== undefined)
                updateData.cinNumber = cinNumber;
            ...
            let settings = await prisma_1.prisma.businessSettings.findFirst();
            if (settings) {
                settings = await prisma_1.prisma.businessSettings.update({
    ...
    // Currency Settings
    async getCurrencySettings(req, res) {
        try {
    ...
    async updateCurrencySettings(req, res) {
        try {
    ...
    async getLeadSources(req, res) {
        try {
            ...
            const mapped = leadSources.map((s, idx) => ({
                id: String(s.id ?? idx),
                name: s.name,
    ...
    async createLeadSource(req, res) {
        try {
            ...
            const exists = await prisma_1.prisma.leadSource.findFirst({
            ...
            const created = await prisma_1.prisma.leadSource.create({
                data: {
    ...
    async getQuotationTemplates(req, res) {
        try {
            const templates = await prisma_1.prisma.quotationTemplate.findMany({
                where: {
                    deletedAt: null,
                },
                orderBy: [
    ...
    async createQuotationTemplate(req, res) {
        try {
            const { name, description, headerContent, footerContent, termsAndConditions, validityDays, showTax, showDiscount, logoPosition, isDefault, isActive, styles, } = req.body;
            // If setting as default, unset other defaults
            if (isDefault) {
                await prisma_1.prisma.quotationTemplate.updateMany({
                    where: { isDefault: true },
                    data: { isDefault: false },
                });
            }
            const template = await prisma_1.prisma.quotationTemplate.create({
                data: {
                    name,
                    description,
                    headerContent,
    ...
    async updateQuotationTemplate(req, res) {
        try {
            const { id } = req.params;
            const { name, description, headerContent, footerContent, termsAndConditions, validityDays, showTax, showDiscount, logoPosition, isDefault, isActive, styles, } = req.body;
            const existingTemplate = await prisma_1.prisma.quotationTemplate.findFirst({
                where: {
                    id: Number(id),
                    deletedAt: null,
                },
            ...
            const template = await prisma_1.prisma.quotationTemplate.update({
                where: { id: Number(id) },
                data: updateData,
            });
            res.json({
    ...
    async deleteQuotationTemplate(req, res) {
        try {
            const { id } = req.params;
            const template = await prisma_1.prisma.quotationTemplate.findFirst({
                where: {
                    id: Number(id),
                    deletedAt: null,
                },
            ...
            await prisma_1.prisma.quotationTemplate.update({
                where: { id: Number(id) },
    ...
    async setDefaultQuotationTemplate(req, res) {
        try {
            const { id } = req.params;
            const template = await prisma_1.prisma.quotationTemplate.findFirst({
                where: {
            ...
            // Set this as default
            const updatedTemplate = await prisma_1.prisma.quotationTemplate.update({
                where: { id: Number(id) },
                data: { isDefault: true },
            });
            res.json({
    ...
    async getProposalTemplates(req, res) {
        try {
    ...
    async getBusinessSettingsFromDB(req, res) {
        try {
            const settings = await prisma_1.prisma.businessSettings.findFirst();
            if (!settings) {
                return res.status(404).json({
    ...
    async updateBusinessSettingsInDB(req, res) {
        try {
            ...
            const { companyName, companyEmail, companyPhone, companyAddress, companyWebsite, companyLogo, timeZone, dateFormat, currency, passwordMinLength, passwordRequireUpper, passwordRequireLower, passwordRequireNumber, passwordRequireSymbol, sessionTimeout, maxLoginAttempts, accountLockDuration, twoFactorRequired, emailVerificationRequired, leadAutoAssignmentEnabled, leadFollowUpReminderDays, 
            // Integration settings
            metaAdsApiKey, metaAdsApiSecret, metaAdsEnabled, indiamartApiKey, indiamartApiSecret, indiamartEnabled, tradindiaApiKey, tradindiaApiSecret, tradindiaEnabled, } = req.body;
            let settings = await prisma_1.prisma.businessSettings.findFirst();
            const updateData = {};
            ...
            if (passwordMinLength !== undefined)
                updateData.passwordMinLength = passwordMinLength;
            if (passwordRequireUpper !== undefined)
                updateData.passwordRequireUpper = passwordRequireUpper;
            if (passwordRequireLower !== undefined)
                updateData.passwordRequireLower = passwordRequireLower;
            if (passwordRequireNumber !== undefined)
                updateData.passwordRequireNumber = passwordRequireNumber;
            if (passwordRequireSymbol !== undefined)
                updateData.passwordRequireSymbol = passwordRequireSymbol;
            if (sessionTimeout !== undefined)
                updateData.sessionTimeout = sessionTimeout;
            if (maxLoginAttempts !== undefined)
                updateData.maxLoginAttempts = maxLoginAttempts;
            if (accountLockDuration !== undefined)
                updateData.accountLockDuration = accountLockDuration;
            if (twoFactorRequired !== undefined)
                updateData.twoFactorRequired = twoFactorRequired;
            if (emailVerificationRequired !== undefined)
                updateData.emailVerificationRequired = emailVerificationRequired;
            if (leadAutoAssignmentEnabled !== undefined)
                updateData.leadAutoAssignmentEnabled = leadAutoAssignmentEnabled;
            if (leadFollowUpReminderDays !== undefined)
                updateData.leadFollowUpReminderDays = leadFollowUpReminderDays;
            // Integration settings
            if (metaAdsApiKey !== undefined)
                updateData.metaAdsApiKey = metaAdsApiKey;
            if (metaAdsApiSecret !== undefined)
                updateData.metaAdsApiSecret = metaAdsApiSecret;
            if (metaAdsEnabled !== undefined)
                updateData.metaAdsEnabled = metaAdsEnabled;
            if (indiamartApiKey !== undefined)
                updateData.indiamartApiKey = indiamartApiKey;
            if (indiamartApiSecret !== undefined)
                updateData.indiamartApiSecret = indiamartApiSecret;
            if (indiamartEnabled !== undefined)
                updateData.indiamartEnabled = indiamartEnabled;
            if (tradindiaApiKey !== undefined)
                updateData.tradindiaApiKey = tradindiaApiKey;
            if (tradindiaApiSecret !== undefined)
                updateData.tradindiaApiSecret = tradindiaApiSecret;
            if (tradindiaEnabled !== undefined)
                updateData.tradindiaEnabled = tradindiaEnabled;
            if (settings) {
                settings = await prisma_1.prisma.businessSettings.update({
                    where: { id: settings.id },
                    data: updateData,
    ...
    async testIntegrationConnection(req, res) {
        try {
    ...
    async syncAllIntegrations(req, res) {
        try {
            const integrationScheduler = new IntegrationScheduler_1.IntegrationScheduler(prisma_1.prisma);
    ...
    async syncSpecificIntegration(req, res) {
        try {
            const { integrationName } = req.params;
            const integrationScheduler = new IntegrationScheduler_1.IntegrationScheduler(prisma_1.prisma);
            await integrationScheduler.initialize();
...
exports.businessSettingsController = new BusinessSettingsController();
//# sourceMappingURL=businessSettingsController.js.map