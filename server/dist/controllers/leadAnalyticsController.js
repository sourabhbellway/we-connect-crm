...
exports.getLeadActivityReport = exports.getLeadConversionFunnel = exports.getLeadTrendAnalytics = exports.getAgentPerformanceAnalytics = exports.getLeadSourceAnalytics = exports.getLeadPipelineAnalytics = exports.getDashboardKPIs = void 0;
...
const getDashboardKPIs = async (req, res) => {
    try {
        ...
        // Revenue Metrics
        const totalRevenue = await prisma_1.prisma.deal.aggregate({
            where: {
                status: "WON",
                isActive: true,
                deletedAt: null,
        ...
        const avgDealSize = await prisma_1.prisma.deal.aggregate({
            where: {
                status: "WON",
                isActive: true,
                deletedAt: null,
        ...
        const [wonDeals, lostDeals, activeDeals, totalLeads, convertedLeads] = await Promise.all([
            prisma_1.prisma.deal.count({ where: { status: "WON", isActive: true, deletedAt: null, ...dateFilter } }),
            prisma_1.prisma.deal.count({ where: { status: "LOST", isActive: true, deletedAt: null, ...dateFilter } }),
            prisma_1.prisma.deal.count({ where: { status: { in: ["DRAFT", "IN_PROGRESS", "NEGOTIATION"] }, isActive: true, deletedAt: null, ...dateFilter } }),
            prisma_1.prisma.lead.count({ where: whereClause }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "CLOSED" } })
        ]);
        // Win Rate
        ...
        const closedDealsWithLeads = await prisma_1.prisma.deal.findMany({
            where: {
                status: "WON",
                isActive: true,
                deletedAt: null,
        ...
        const [totalCallLogs, totalTasks, completedTasks] = await Promise.all([
            prisma_1.prisma.callLog.count({ where: { createdAt: dateFilter.createdAt || {} } }),
            prisma_1.prisma.task.count({ where: { createdAt: dateFilter.createdAt || {} } }),
            prisma_1.prisma.task.count({ where: { status: "COMPLETED", createdAt: dateFilter.createdAt || {} } })
        ]);
        const taskCompletionRate = totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0;
        // Revenue by Source
        const revenueBySource = await prisma_1.prisma.leadSource.findMany({
            include: {
                leads: {
                    where: {
                        isActive: true,
        ...
        const leadsWithContacted = await prisma_1.prisma.lead.findMany({
            where: {
                ...whereClause,
...
const getLeadPipelineAnalytics = async (req, res) => {
    try {
        ...
        if (sourceId) {
            whereClause.sourceId = parseInt(sourceId);
        }
        // Pipeline stage analysis
        const pipelineData = await Promise.all([
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "NEW" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "CONTACTED" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "QUALIFIED" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "PROPOSAL" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "NEGOTIATION" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "CLOSED" } }),
            prisma_1.prisma.lead.count({ where: { ...whereClause, status: "LOST" } }),
        ]);
        const stages = [
...
exports.getLeadPipelineAnalytics = getLeadPipelineAnalytics;
const getLeadSourceAnalytics = async (req, res) => {
    try {
        ...
        const sourceAnalytics = await prisma_1.prisma.leadSource.findMany({
            include: {
                _count: {
                    select: {
                        leads: {
        ...
        const leadsWithoutSource = await prisma_1.prisma.lead.count({
            where: {
                ...whereClause,
                sourceId: null
            }
        });
        const closedLeadsWithoutSource = await prisma_1.prisma.lead.count({
            where: {
                ...whereClause,
                sourceId: null,
                status: "CLOSED"
...
const getAgentPerformanceAnalytics = async (req, res) => {
    try {
        ...
        const agentPerformance = await prisma_1.prisma.user.findMany({
            where: {
                assignedLeads: {
                    some: whereClause
                }
        ...
        const detailedPerformance = await Promise.all(agentPerformance.map(async (agent) => {
            const [totalLeads, contactedLeads, qualifiedLeads, closedLeads, avgResponseTime, followUps, communications] = await Promise.all([
                ...
                prisma_1.prisma.leadFollowUp.count({
                    where: {
                        userId: agent.id,
                        isCompleted: true,
                        ...(startDate || endDate ? {
                ...
                prisma_1.prisma.leadCommunication.count({
                    where: {
                        userId: agent.id,
                        ...(startDate || endDate ? {
                            createdAt: {
...
exports.getAgentPerformanceAnalytics = getAgentPerformanceAnalytics;
const getLeadTrendAnalytics = async (req, res) => {
    try {
...
exports.getLeadTrendAnalytics = getLeadTrendAnalytics;
const getLeadConversionFunnel = async (req, res) => {
    try {
        const { startDate, endDate, sourceId } = req.query;
        let dateFilter = {};
        if (startDate || endDate) {
...
exports.getLeadConversionFunnel = getLeadConversionFunnel;
const getLeadActivityReport = async (req, res) => {
    try {
        ...
        if (assignedTo) {
            whereClause.userId = parseInt(assignedTo);
        }
        if (leadId) {
            whereClause.leadId = parseInt(leadId);
        }
        // Get follow-ups
        const followUps = await prisma_1.prisma.leadFollowUp.findMany({
            where: whereClause,
            include: {
                user: {
                    select: {
        ...
        // Get communications
        const communications = await prisma_1.prisma.leadCommunication.findMany({
            where: whereClause,
            include: {
                user: {
                    select: {
...
exports.getLeadActivityReport = getLeadActivityReport;
//# sourceMappingURL=leadAnalyticsController.js.map