...
export const getTasks = async (req: Request, res: Response) => {
  try {
    ...
    if (dealId) {
      whereClause.dealId = parseInt(dealId as string);
    }
    if (contactId) {
...
export const createTask = async (req: Request, res: Response) => {
  try {
    ...
    // Log activity
    await activityLoggers.taskCreated(
      {
        id: task.id,
        title: task.title,
...
export const updateTask = async (req: Request, res: Response) => {
  try {
    ...
    if (isNowCompleted && !wasCompleted) {
      await activityLoggers.taskCompleted(
        {
          id: task.id,
          title: task.title,
          leadId: task.leadId,
...
export const getTaskStats = async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      prisma.task.count({ where: { status: "PENDING", isActive: true } }),
      prisma.task.count({ where: { status: "IN_PROGRESS", isActive: true } }),
      prisma.task.count({ where: { status: "COMPLETED", isActive: true } }),
      prisma.task.count({ 
        where: { 
          isActive: true, 
          dueDate: { lt: new Date() },
