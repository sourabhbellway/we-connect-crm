...
let LeadsService = class LeadsService {
    ...
    async getById(id, user) {
        ...
        const payments = leadRow.invoices?.flatMap((inv) => inv.payments?.map((p) => ({
            ...p,
    ...
    async update(id, dto) {
        ...
        try {
            ...
            const description = changes.length > 0 ? changes.join(', ') : 'Lead information updated';
            await this.prisma.activity.create({
                data: {
                    title,
                    description,
        ...
        try {
            ...
            if (rest.status && lead.status !== updated.status) {
                await this.automationService.executeWorkflowsForTrigger(create_workflow_dto_1.WorkflowTrigger.LEAD_STATUS_CHANGED, {
                    ...normalized,
                    entityType: 'lead',
                    oldStatus: lead.status,
                    newStatus: updated.status,
            ...
            if (rest.assignedTo !== undefined &&
                lead.assignedTo !== updated.assignedTo) {
                await this.automationService.executeWorkflowsForTrigger(create_workflow_dto_1.WorkflowTrigger.LEAD_ASSIGNED, {
                    ...normalized,
                    entityType: 'lead',
                    oldAssignedTo: lead.assignedTo,
                    newAssignedTo: updated.assignedTo,
    ...
    async convert(id, dto) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (dto.createDeal) {
                const fallbackTitleBase = `${lead.firstName || ''} ${lead.lastName || ''}`.trim() ||
                    `Lead #${lead.id}`;
    ...
    async undoLeadConversion(id) {
        ...
        if (!lead) {
            return {
                success: false,
                message: 'This lead cannot be reverted. It may not have been converted or the conversion link is missing.',
            };
        }
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                try {
                    ...
                    Object.keys(row).forEach((key) => {
                        const standardKey = standardFields.find((f) => f
                            .toLowerCase()
                            .replace(/\s+/g, '')
                            .replace(/[^a-z0-9]/g, '') ===
                            key
                    ...
                    if (leadData.annualRevenue !== undefined &&
