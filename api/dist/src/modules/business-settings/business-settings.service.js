...
let BusinessSettingsService = class BusinessSettingsService {
        ...
        try {
            ...
            const updated = await this.prisma.businessSettings.update({
                ...
                data: {
                    ...
                    preferences: currentPreferences ?? undefined,
                },
            });
    ...
    async getIntegrationsStatus() {
        ...
        for (const name of list) {
            ...
            const item = cfg[key] || {};
            const configured = Boolean(item.apiKey || item.apiKey === '' ? item.apiKey : false) ||
    ...
    async updateIntegrationSettings(body) {
        const bs = await this.ensureSettings();
        let ext = {};
        try {
            ext = bs.description ? JSON.parse(bs.description) : {};
    ...
    async createQuotationTemplate(body) {
        if (body.isDefault) {
    ...
    async updateQuotationTemplate(id, body) {
        if (body.isDefault) {
            await this.prisma.quotationTemplate.updateMany({
                where: { isDefault: true },
                data: { isDefault: false },
    ...
    async setDefaultQuotationTemplate(id) {
        await this.prisma.quotationTemplate.updateMany({
            where: { isDefault: true },
            data: { isDefault: false },
        });
    ...
    async updateEmailTemplate(id, body) {
        try {
            if (body.isDefault) {
    ...
    async setDefaultEmailTemplate(id, category) {
        await this.prisma.emailTemplate.updateMany({
            where: {
                category: category,
                isDefault: true,
    ...
    async getEmailTemplatesByCategory(category) {
        const templates = await this.prisma.emailTemplate.findMany({
            where: {
                category: category,
                isActive: true,
    ...
    async getSystemEmailTemplate(category) {
        const template = await this.prisma.emailTemplate.findFirst({
            where: {
                category: category,
                isDefault: true,
    ...
    async upsertWelcomeEmailTemplate(body) {
        if (body.isDefault) {
            await this.prisma.emailTemplate.updateMany({
    ...
    async previewEmailTemplate(id, sampleData) {
        ...
        const renderTemplate = (content) => {
            return content.replace(/\{(\w+)\}/g, (_, key) => defaultData[key] ?? `{${key}}`);
        };
        const preview = {
            id: template.id,
            name: template.name,
    ...
    async updateFieldConfig(id, body) {
    ...
    async deleteFieldConfig(id) {
        ...
        if (field) {
            ...
            if (protectedFields.includes(field.fieldName)) {
                throw new common_1.BadRequestException('Cannot delete core system fields');
            }
        }
    ...
    async createTermsAndConditions(body) {
        if (body.isDefault) {
    ...
    async setDefaultTermsAndConditions(id) {
        await this.prisma.termsAndConditions.updateMany({
            where: { isDefault: true },
            data: { isDefault: false },
        });
