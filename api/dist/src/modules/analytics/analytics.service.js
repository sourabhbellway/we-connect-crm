...
let AnalyticsService = class AnalyticsService {
    ...
    async getDealVelocity(months = 6, userId, includeTeamData = false, currentUser) {
        ...
        closedDeals.forEach((deal) => {
            if (deal.lead?.createdAt && deal.actualCloseDate) {
                const month = new Date(deal.actualCloseDate).toISOString().slice(0, 7);
                const days = Math.floor((deal.actualCloseDate.getTime() - deal.lead.createdAt.getTime()) /
                    (1000 * 60 * 60 * 24));
                if (!monthlyData[month]) {
    ...
    async getTaskReport(months = 6, userId, scope = 'all', currentUser, page = 1, limit = 10, filters) {
        ...
        const teamPerformanceData = members
            ...
            const overdue = user.assignedTasks.filter((t) => t.status !== 'COMPLETED' &&
                t.dueDate &&
    ...
    async getDealReport(months = 6, userId, scope = 'all', currentUser, page = 1, limit = 10, filters) {
        ...
        const avgDaysToClose = wonDealsWithDates.length > 0
            ? wonDealsWithDates.reduce((sum, d) => sum +
                (d.actualCloseDate.getTime() - d.createdAt.getTime()) /
                    (1000 * 60 * 60 * 24), 0) / wonDealsWithDates.length
            : 0;
