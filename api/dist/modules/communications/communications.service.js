...
let CommunicationsService = class CommunicationsService {
    ...
    async createTemplate(dto) {
        if (dto.isDefault) {
            await this.prisma.communicationTemplate.updateMany({ where: { type: dto.type, isDefault: true }, data: { isDefault: false } });
        }
    ...
    async sendWhatsApp(dto) {
        const message = await this.prisma.communicationMessage.create({
            data: {
                ...
                status: 'SENT',
                sentAt: new Date(),
                metadata: (dto.mediaUrls ? { mediaUrls: dto.mediaUrls } : undefined),
            },
        });
