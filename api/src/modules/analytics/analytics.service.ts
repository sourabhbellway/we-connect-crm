...
export class AnalyticsService {
  ...
  async kpis(startDate?: string, endDate?: string) {
    ...
    const revenueBySource = revenueBySourceRaw
      .map((src: any) => {
        const total = src.leads.reduce((sum: number, lead: any) => sum + lead.deals.reduce((ds: number, deal: any) => ds + (deal.value ? Number(deal.value) : 0), 0), 0);
    ...
    if (leadsWithContacted.length > 0) {
      const totalHours = leadsWithContacted.reduce((sum: number, lead: any) => sum + ((lead.lastContactedAt!.getTime() - lead.createdAt.getTime()) / (1000 * 60 * 60)), 0);
