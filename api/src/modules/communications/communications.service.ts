...
export class CommunicationsService {
  ...
  async listLeadComms(leadId: number) {
    const items = await this.prisma.leadCommunication.findMany({ where: { leadId }, orderBy: { createdAt: 'desc' } });
  ...
  // Templates
  async listTemplates({ type, active, page = 1, limit = 10 }: { type?: string; active?: string; page?: number; limit?: number }) {
    ...
    return { success: true, data: { templates, pagination: { currentPage: page, totalPages: Math.ceil(total / limit), totalItems: total, itemsPerPage: limit } } };
  }

  async createTemplate(dto: UpsertTemplateDto) {
    if (dto.isDefault) {
      await this.prisma.communicationTemplate.updateMany({ where: { type: dto.type as any, isDefault: true }, data: { isDefault: false } });
    }
  ...
  async updateTemplate(id: number, dto: UpsertTemplateDto) {
    if (dto.isDefault) {
      ...
      if (existing) {
        await this.prisma.communicationTemplate.updateMany({ where: { type: existing.type, isDefault: true, id: { not: id } }, data: { isDefault: false } });
      }
    }
  ...
  async deleteTemplate(id: number) {
    await this.prisma.communicationTemplate.update({ where: { id }, data: { deletedAt: new Date() } });
  ...
  async sendEmail(dto: SendEmailDto) {
    ...
    return { success: true, message: 'Email sent successfully', data: { messageId: message.id } };
  }
  ...
  async sendWhatsApp(dto: SendWhatsAppDto) {
    const message = await this.prisma.communicationMessage.create({
      data: {
        ...
        status: 'SENT' as any,
        sentAt: new Date(),
        metadata: (dto.mediaUrls ? { mediaUrls: dto.mediaUrls } : undefined) as any,
      },
    });
    return { success: true, message: 'WhatsApp message sent successfully', data: { messageId: message.id } };
  }
  ...
  async sendTemplated(dto: SendTemplatedDto) {
    ...
    if (template.type === 'EMAIL') {
      return this.sendEmail({ leadId: dto.leadId, to: '', subject: template.subject || 'Message', content, templateId: template.id, userId: dto.userId });
    }
    if (template.type === 'WHATSAPP') {
      return this.sendWhatsApp({ leadId: dto.leadId, to: '', content, templateId: template.id, userId: dto.userId });
    }
    return { success: false, message: `Template type ${template.type} not supported` };
  }

  async listMessages({ leadId, type, status, page = 1, limit = 10 }: { leadId?: number; type?: string; status?: string; page?: number; limit?: number }) {
    const where: any = {};
    ...
    return { success: true, data: { messages, pagination: { currentPage: page, totalPages: Math.ceil(total / limit), totalItems: total, itemsPerPage: limit } } };
  }
}
