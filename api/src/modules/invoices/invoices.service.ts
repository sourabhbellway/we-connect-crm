        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (templateId === 'professional') {
                    ...
                    if (invoice.dueDate) doc.text(`Due Date: ${new Date(invoice.dueDate).toLocaleDateString()}`, 40, 115);
                    ...
                    if (companyPhone) { doc.text(companyPhone, rightColX, companyY, { align: 'right', width: 205 }); companyY += 12; }
                    if (companyEmail) { doc.text(companyEmail, rightColX, companyY, { align: 'right', width: 205 }); companyY += 12; }
                    if (gstNo && gstNo !== 'N/A') { doc.text(`GSTIN: ${gstNo}`, rightColX, companyY, { align: 'right', width: 205 }); companyY += 12; }
                    if (panNo && panNo !== 'N/A') { doc.text(`PAN: ${panNo}`, rightColX, companyY, { align: 'right', width: 205 }); companyY += 12; }
                    if (cinNo && cinNo !== 'N/A') { doc.text(`CIN: ${cinNo}`, rightColX, companyY, { align: 'right', width: 205 }); }
                    ...
                    if (customerCompany) { doc.text(customerCompany, 40, clientAddrY); clientAddrY += 14; }
                    if (customerAddress) {
                    ...
                    if (footerY > 700) { doc.addPage(); footerY = 50; }
                    ...
                    if (bs?.description && bs.description.includes('bankName')) {
                        ...
                        doc.text(`Bank: ${bank.bankName || '-'}`, 40, footerY); footerY += 14;
                        doc.text(`Account No: ${bank.accountNumber || '-'}`, 40, footerY); footerY += 14;
                    ...
                    if (invoice.dueDate) drawMetaLine('Due Date:', new Date(invoice.dueDate).toLocaleDateString(), metaY + 30);
                    ...
                    invoice.items.forEach((item, idx) => {
                        const itemHeight = 25;
                        if (rowY + itemHeight > 750) { doc.addPage(); rowY = 40; }

                        // Row background
                        if (idx % 2 !== 0) doc.rect(40, rowY, itemsTableWidth, itemHeight).fill(lightGray);
                    ...
                    doc.fontSize(9).font('Helvetica-Bold').fillColor('black');
                    doc.text('Payment Method:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text('Bank Transfer', 120, rowY); rowY += 15;
                    doc.font('Helvetica-Bold').fillColor('black').text('Due Date:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text(invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : 'N/A', 120, rowY); rowY += 15;

                    if (bs?.description && bs.description.includes('accountNumber')) {
                        try {
                            const bank = JSON.parse(bs.description);
                            doc.font('Helvetica-Bold').fillColor('black').text('Bank Account:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text(`${bank.bankName || ''} - ${bank.accountNumber || ''} `, 120, rowY);
                        } catch { }
                    } else {
                        doc.font('Helvetica-Bold').fillColor('black').text('Bank Account:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text('1234-5678-9012-3456', 120, rowY);
                    }
                    ...
                    rowY += 20;
                    doc.fontSize(9).font('Helvetica-Bold').text('Email US:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text(companyEmail || 'N/A', 90, rowY); rowY += 13;
                    doc.font('Helvetica-Bold').fillColor('black').text('Call US:', 40, rowY); doc.font('Helvetica').fillColor(textGray).text(companyPhone || 'N/A', 90, rowY);
                    ...
                    if (gstNo && gstNo !== 'N/A') { doc.text(`GSTIN: ${gstNo} `, leftX, leftY); leftY += 12; }
                    if (panNo && panNo !== 'N/A') { doc.text(`PAN: ${panNo} `, leftX, leftY); leftY += 12; }
                    if (cinNo && cinNo !== 'N/A') { doc.text(`CIN: ${cinNo} `, leftX, leftY); leftY += 12; }
