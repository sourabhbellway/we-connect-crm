    ...
    try {
      ...
      const invoice = await this.prisma.invoice.create({
        data: {
          ...
          status: (dto.status ? dto.status.toUpperCase() : 'DRAFT') as any,
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const customerAddress =
          invoice.lead?.address || invoice.lead?.city || '';
        ...
        if (templateId === 'professional') {
          ...
          invoice.items.forEach((item, idx) => {
            ...
            doc.text(
              ...
              cols.rate.x,
              rowY + 12,
              { width: cols.rate.width, align: 'right' },
            );
            doc
              .font('Helvetica-Bold')
              .text(
          ...
          templateId === 'classic-right' ||
          templateId === 'template2'
        ) {
          // --- TEAL PROFESSIONAL TEMPLATE (Template 2) ---
          // Replaced hardcoded "BRAND NAME" with actual company details and fixed branding element positions
          const primaryAccent = '#00897B'; // Teal
          const secondaryAccent = '#E0F2F1'; // Light Teal
          ...
          // Styled Header Bar for "Invoice To"
          ...
          // --- FOOTER SECTION: Payment Info & Terms ---
          let footerY = rowY + 60;
          ...
          doc.fontSize(9).font('Helvetica').fillColor(grayText);
          footerY += 20;
          // Example bank details - in real app should come from settings
          ...
          // Draw Notes at the very bottom
          drawNotesAndTerms(footerY + 50);
        } else if (
          templateId === 'template1' ||
          templateId === 'standard-left'
        ) {
          // --- Template 1: PREMIUM MODERN LAYOUT (Based on Design Studio Image) ---
          ...
          let termsY = rowY - 60; // Pull up slightly beside totals if there is space
          if (termsY < headerY + 50) termsY = rowY + 30; // Push down if too high
          ...
          invoice.items.forEach((item, idx) => {
            ...
            doc
              ...
              .text(
                ...
                colSetup.total.x - 10,
                rowY + 8,
                { width: colSetup.total.width, align: 'right' },
              );
            doc.font('Helvetica');
            doc
          ...
          doc
            ...
            .fillColor(primaryColor)
            .text(
              `${invoice.currency} ${Number(invoice.totalAmount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} `,
              450,
              rowY - 5,
