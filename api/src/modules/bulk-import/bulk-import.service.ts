...
export class BulkImportService {
  ...
  async createLeadBatch(dto: CreateLeadImportBatchDto) {
    const batch = await this.prisma.leadImportBatch.create({
      data: {
        ...
        records: {
          create: dto.records.map((raw, idx) => ({ rowIndex: idx, rawData: raw })) as any,
        },
      },
  ...
  async listBatches({ page = 1, limit = 10 }: { page?: number; limit?: number }) {
    const [items, total] = await Promise.all([
      this.prisma.leadImportBatch.findMany({ skip: (page - 1) * limit, take: limit, orderBy: { createdAt: 'desc' } }),
  ...
  async listRecords(batchId: number) {
    const items = await this.prisma.leadImportRecord.findMany({ where: { batchId }, orderBy: { rowIndex: 'asc' } });
