...
export async function getRoleBasedWhereClause(
  userId: number,
  prisma: any,
  fields: string[] = ['assignedTo', 'createdBy'],
) {
...
export async function getAccessibleUserIds(
  ...
  const hasGlobal = roles.some(
    (r: any) => r.accessScope === RoleAccessScope.GLOBAL,
  );
  if (hasGlobal) return null; // Null means global access

  const hasTeam = roles.some(
    (r: any) => r.accessScope === RoleAccessScope.TEAM,
  );
