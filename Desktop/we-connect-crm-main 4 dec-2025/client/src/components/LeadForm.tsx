...
const LeadForm: React.FC<LeadFormProps> = ({
  ...
  const validatePhone = (value: string): string | null => {
    ...
    for (const country of countries) {
      if (v.startsWith(country.phoneCode)) {
        const codeDigitsLength = country.phoneCode.replace(/\D/g, "").length;
        if (codeDigitsLength > matchedPhoneCodeDigitsLength) {
          matchedCountryCode = country.code;
          matchedPhoneCodeDigitsLength = codeDigitsLength;
        }
      }
    }

    // If the number matches a country from the dropdown, always use a rule
    if (matchedCountryCode) {
      const rule = phoneDigitRules[matchedCountryCode] || defaultPhoneRule;
      const localDigitsLength = digitsOnly.slice(matchedPhoneCodeDigitsLength).length;
      if (localDigitsLength < rule.min) {
        return `Phone must contain at least ${rule.min} digits for ${matchedCountryCode}`;
      }
      if (rule.max && localDigitsLength > rule.max) {
        return `Phone must not exceed ${rule.max} digits for ${matchedCountryCode}`;
      }
      return null;
    }

    // Fallback when we cannot detect a country code (e.g. plain local number)
    if (digitsOnly.length < defaultPhoneRule.min || digitsOnly.length > defaultPhoneRule.max) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg">
      <form onSubmit={handleSubmit} className="space-y-8" noValidate>
        ...
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-100 dark:border-gray-600">
          ...
          {(() => {
            const config = getFieldConfig('lastName');
            return config?.isVisible !== false && (
              <InputField
                label={config?.label || "Last Name"}
                leftIcon={<UserIcon className="h-4 w-4 text-gray-400" />}
          ...
          <div className="md:col-span-2 lg:col-span-2">
            {(() => {
              const config = getFieldConfig('phone');
              return config?.isVisible !== false && (
                <PhoneInputWithCountry
                  label={config?.label || "Phone"}
        ...
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-100 dark:border-gray-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-6">
            {(() => {
              const config = getFieldConfig('budget');
              return config?.isVisible !== false && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {config?.label || "Expected Budget"}
