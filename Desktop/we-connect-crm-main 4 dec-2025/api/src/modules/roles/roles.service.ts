...
export class RolesService {
  ...
  async list({
    ...
    const [items, total, totalPermissions] = await Promise.all([
      this.prisma.role.findMany({
        ...
        include: {
          ...
          // Include user-role assignments so we can show user counts per role
          users: true,
        },
      }),
      this.prisma.role.count({ where }),
      // Total permissions in the system (for Admin role display)
    ...
    const data = items.map((r) => {
      ...
      return {
        ...
        permissions,
        // expose raw assignments for UI if needed
        users: r.users,
        // dynamic counts used by frontend cards
  ...
  async update(id: number, dto: UpsertRoleDto) {
    ...
    // If deactivating role, check if any users are assigned
    let warning = null;
    ...
    if (dto.isActive === false) {
      ...
      if (usersWithRole > 0) {
        warning = "This role is assigned to users. Those users will not be able to log in.";
      }
    }
