...
export class LeadsService {
  ...
  async getStats() {
    ...
    const statusMap: Record<LeadStatus, keyof typeof summary | null> = {
      [LeadStatus.NEW]: 'newLeads',
      [LeadStatus.CONTACTED]: 'contactedLeads',
      [LeadStatus.QUALIFIED]: 'qualifiedLeads',
      [LeadStatus.PROPOSAL]: 'proposalLeads',
      [LeadStatus.NEGOTIATION]: 'negotiationLeads',
      [LeadStatus.CLOSED]: 'closedLeads',
      [LeadStatus.LOST]: 'lostLeads',
      [LeadStatus.CONVERTED]: 'convertedLeads',
    };

    grouped.forEach((group) => {
      const key = statusMap[group.status as LeadStatus];
      if (key) {
        summary[key] += group._count._all;
      }
    });

    summary.closedLeads += summary.convertedLeads;
    summary.activeLeads = Math.max(
      0,
      totalLeads - summary.convertedLeads - summary.lostLeads,
    );
