...
const Leads: React.FC = () => {
  ...
  const handleStatusChange = async (leadId: number, newStatus: string) => {
    ...
    try {
      ...
      if (shouldConvert) {
        // Find the lead to get conversion data
        const lead = leads.find(l => l.id === leadId);
        if (!lead) {
          throw new Error('Lead not found');
        }

        // Prepare conversion data with defaults
        const conversionData: ConversionData = {
          ...
          dealData: {
            ...
            value: (lead as any)?.budget ?? undefined,
            currency: (lead as any)?.currency || "USD",
            status: "DRAFT",
            probability: 50,
          },
        };
        ...
        // Convert the lead
        await leadService.convertLead(leadId, conversionData);

        // Update the leads array locally with converted status
        setLeads(prev => prev.map(lead =>
          lead.id === leadId ? { ...lead, status: 'converted' as Lead['status'] } : lead
        ));
