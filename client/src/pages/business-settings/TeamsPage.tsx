    ...
    if (team) {
      ...
      setFormData({
        ...
        memberIds:
          team.members
            ?.map((m) => m.id.toString())
            .filter((id) => id !== team.managerId?.toString()) || [],
              ...
              <div>
                ...
                <div className="border border-gray-300 dark:border-gray-600 rounded-lg max-h-48 overflow-y-auto divide-y divide-gray-200 dark:divide-gray-700">
                  {users
                    .filter(
                      (user) =>
                        (!user.teamId || (editTeam && user.teamId === editTeam.id)) &&
                        user.id.toString() !== formData.managerId
                    )
                    .map((user) => (
                      <div
                        key={user.id}
                        className="flex items-center p-2 hover:bg-gray-50 dark:hover:bg-gray-700 gap-3"
                      >
                        <input
                          type="checkbox"
                          ...
                          onChange={() => {
                            ...
                            setFormData((prev) => {
                              ...
                              if (currentMembers.includes(userId)) {
                                return {
                                  ...prev,
                                  memberIds: currentMembers.filter((id) => id !== userId),
                                };
                              } else {
                        ...
                        <span className="text-sm text-gray-700 dark:text-gray-300">
                          ...
                          {user.team && (
                            <span className="text-xs text-gray-400 ml-1">({user.team.name})</span>
                          )}
                        </span>
                      </div>
                    ))}
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-200 dark:divide-gray-700 max-h-60 overflow-y-auto">
                  {viewTeam.members && viewTeam.members.length > 0 ? (
                    viewTeam.members.map((member) => (
                      <div
                        key={member.id}
                        className="p-3 flex items-center hover:bg-gray-50 dark:hover:bg-gray-700/50"
                      >
