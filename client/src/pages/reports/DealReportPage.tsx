    ...
    const fetchMetadata = async () => {
      try {
        ...
        if (usersRes.success) {
          ...
          if (userField) {
            userField.options = usersRes.data.map((u: any) => ({
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="GHOST" size="SM" onClick={() => navigate("/reports")} className="p-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 font-medium">
              Comprehensive overview of sales pipeline and closure rates.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
            ...
            {["3months", "6months", "12months"].map((range) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
                  dateRange === range && !useCustomRange
          ...
          <div className="flex items-center gap-2">
            <div
              className={`flex items-center gap-1 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-1 ${useCustomRange ? "border-blue-500" : ""}`}
            >
              ...
              {chartType === "area" ? (
                <AreaChart data={reportData?.salesTrendData || []}>
                  ...
                  <YAxis
                    stroke="#9CA3AF"
                    tickFormatter={(value) =>
                      `${reportData?.currency?.symbol || "$"}${value >= 1000 ? (value / 1000).toFixed(0) + "K" : value}`
                    }
                  />
            ...
            <div className="grid grid-cols-2 gap-4 h-[300px]">
              <div className="flex flex-col items-center justify-center p-6 bg-blue-50 dark:bg-blue-900/20 rounded-2xl">
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white">
                  {reportData?.stats?.avgDaysToClose || 0}
                </p>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {(reportData?.recentClosedDeals || []).map((deal: any, i: number) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">
                      {deal.date}
                    </td>
                  </tr>
                ))}
                {(reportData?.recentClosedDeals || []).length === 0 && (
                  <tr>
                    <td
                      colSpan={4}
                      className="px-6 py-10 text-center text-gray-400 font-medium italic"
                    >
