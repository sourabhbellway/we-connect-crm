...
const InvoiceReportPage: React.FC = () => {
  ...
  const stats = [
    {
      ...
      value: reportData?.stats?.totalBilled || "0",
      icon: DollarSign,
      color: "text-blue-500",
      bg: "bg-blue-50",
    },
    {
      label: "Amount Paid",
      value: reportData?.stats?.totalPaid || "0",
      icon: CheckCircle,
    ...
    {
      label: "Outstanding",
      value: reportData?.stats?.outstanding || "0",
      icon: Clock,
    ...
    {
      label: "Overdue",
      value: reportData?.stats?.overdue || "0",
      icon: AlertCircle,
      color: "text-red-500",
      bg: "bg-red-50",
    },
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `high_value_invoices_${new Date().toISOString().slice(0, 10)}.csv`
    );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 font-medium">
              Financial insights, collection trends, and receivables.
            </p>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={reportData?.revenueTrendData || []}>
                ...
                <YAxis
                  ...
                  tickFormatter={(value) =>
                    value >= 1000
                      ? `${reportData?.currency?.symbol || "$"}${(value / 1000).toFixed(0)}K`
                      : `${reportData?.currency?.symbol || "$"}${value}`
                  }
                />
