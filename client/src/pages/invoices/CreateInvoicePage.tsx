  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setDate(inv.createdAt ? String(inv.createdAt).slice(0, 10) : date);
        setNotes(inv.notes || '');
        setTerms(inv.terms || '');
        setEmail(inv.lead?.email || email);
        if (inv.lead) {
          setRelatedType('lead');
          setRelatedId(String(inv.leadId));
          const full = `${inv.lead.firstName || ''} ${inv.lead.lastName || ''}`.trim();
          setSearchQuery(full);
        }
        // Map items for UI
      ...
      try {
        if (entityType === 'lead') {
          ...
          if (lead) {
            ...
            setSubject(`Invoice for ${lead.firstName} ${lead.lastName}`);
          ...
          if (deal) {
            ...
            setSubject(`Invoice for ${deal.title || 'Deal'}`);
          }
        }
      } catch (e) {
  ...
  const handleSubmit = async (sendImmediately = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        notes,
        terms,
        leadId: relatedType === 'lead' ? parseInt(relatedId) : null,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
      <div className="mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Create Invoice</h1>
