...
const TrashPage: React.FC = () => {
  ...
  const getEntityIcon = (type: string) => {
    switch (type) {
      ...
      case "invoice":
        return <Receipt className="text-emerald-500" size={18} />;
      case "product":
        return <Package className="text-orange-500" size={18} />;
      case "lead":
        return <User className="text-rose-500" size={18} />;
      default:
        return <Trash2 className="text-gray-500" size={18} />;
    }
  };
  ...
  const categories = [
    {
      id: "user",
      name: "Deleted Users",
      desc: "Manage soft-deleted user accounts",
      icon: Users,
      color: "blue",
      count: stats.user,
      label: "users",
    },
    {
      id: "lead",
      name: "Deleted Leads",
      desc: "Review removed lead records",
      icon: User,
      color: "green",
      count: stats.lead,
      label: "leads",
    ...
    {
      id: "quotation",
      name: "Deleted Quotations",
      desc: "Manage soft-deleted quotations",
      icon: FileText,
      color: "indigo",
    ...
    },
    {
      ...
      desc: "Manage soft-deleted invoices",
      icon: Receipt,
      color: "emerald",
      count: stats.invoice,
      label: "invoices",
    },
    {
      ...
      desc: "Manage soft-deleted products",
      icon: Package,
      color: "orange",
      count: stats.product,
      label: "products",
        ...
        {categories.map((cat) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div
                className={`p-3 rounded-xl bg-${cat.color}-100 dark:bg-${cat.color}-900/30 text-${cat.color}-600 dark:text-${cat.color}-400`}
              >
                <cat.icon size={24} />
              </div>
              <ChevronRight
                className={`text-gray-400 group-hover:translate-x-1 transition-transform ${typeFilter === cat.id ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                size={18}
              />
            </div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 mb-6">{cat.desc}</p>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-slate-800/50">
                  {items.map((item) => (
                    <tr
                      ...
                      className="group hover:bg-gray-50/50 dark:hover:bg-slate-800/20 transition-colors"
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-gray-400 flex items-center gap-1 mt-0.5">
                              ...
                              {format(new Date(item.deletedAt), "MMM dd, h:mm a")}
                            </div>
                          </div>
                        </div>
                      </td>
          ...
          {pagination.totalPages > 1 && (
            <div className="p-4 bg-gray-50/50 dark:bg-slate-800/20 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <p className="text-xs text-gray-400">
                showing{" "}
                <span className="text-gray-900 dark:text-white font-bold">{items.length}</span> of{" "}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between p-6 bg-red-500/5 border border-red-500/10 rounded-2xl gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 dark:text-red-500/60">
              Cleaning the trash will permanently remove all items from our database forever.
            </p>
          </div>
        </div>
        {/* <button
