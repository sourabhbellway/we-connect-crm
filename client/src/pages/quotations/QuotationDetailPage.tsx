...
const QuotationDetailPage: React.FC = () => {
  ...
  const handleAction = async (action: "accept" | "reject" | "send") => {
    ...
    try {
      ...
      switch (action) {
        case "accept":
          endpoint = `/quotations/${id}/accept`;
          successMsg = "Quotation marked as accepted";
          break;
        case "reject":
          endpoint = `/quotations/${id}/reject`;
          successMsg = "Quotation marked as rejected";
          break;
        case "send":
          endpoint = `/quotations/${id}/send`;
          successMsg = "Quotation marked as sent";
          break;
      }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                ...
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${status.color}`}
                >
                  {status.label}
                </span>
              </h1>
          ...
          <div className="p-12 print:p-8">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-8 mb-16">
              <div className="space-y-6 max-w-md">
                {businessSettings?.company?.logo ? (
                  <img
                    src={
                      businessSettings.company.logo.startsWith("data:image")
                        ? businessSettings.company.logo
                ...
                <div className="space-y-1.5 text-sm text-slate-500 dark:text-slate-400 font-medium">
                  <p className="text-slate-900 dark:text-slate-200 font-bold text-base mb-2">
                    {businessSettings?.company?.name}
                  </p>
                  {businessSettings?.company?.address && (
                    <p className="flex items-start gap-2 max-w-[320px]">
                      <MapPin className="w-3.5 h-3.5 mt-1 shrink-0 text-slate-400" />{" "}
                      {businessSettings.company.address}
                    </p>
                  )}
                  {businessSettings?.company?.email && (
                    <p className="flex items-center gap-2">
                      <Mail className="w-3.5 h-3.5 text-slate-400" />{" "}
                      {businessSettings.company.email}
                    </p>
                  )}
                  {businessSettings?.company?.phone && (
                    <p className="flex items-center gap-2">
                    ...
                    <div className="pt-6 flex flex-wrap gap-x-3 gap-y-2">
                      {businessSettings.company.gstNumber && (
                        <div className="bg-slate-100/80 dark:bg-slate-800/80 px-3 py-1.5 rounded-lg text-[10px] font-black border border-slate-200/50 dark:border-slate-700/50">
                          <span className="text-indigo-600 dark:text-indigo-400 uppercase mr-2 tracking-widest">
                            GSTIN
                          </span>
              ...
              <div className="text-left md:text-right space-y-4">
                <h2 className="text-7xl font-black text-slate-900 dark:text-white tracking-tighter opacity-[0.03] print:opacity-[0.05] uppercase absolute right-12 top-24 pointer-events-none select-none">
                  Quotation
                </h2>
                <div className="space-y-1 pt-6 relative">
                  ...
                  <p className="text-4xl font-black text-indigo-600 tracking-tighter leading-none py-2">
                    {quotation.quotationNumber}
                  </p>
                </div>
              ...
              <div className="space-y-8">
                ...
                {quotation.lead ? (
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-2">
                        {quotation.lead.firstName} {quotation.lead.lastName}
                      </h3>
                      {quotation.lead.company && (
                        <p className="text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-[0.1em]">
                          {quotation.lead.company}
                        </p>
                      )}
                    </div>
                    <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400 font-semibold">
                      ...
                      <p className="flex items-center gap-3">
                        <Mail className="w-4 h-4 shrink-0 text-slate-300" /> {quotation.lead.email}
                      </p>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-900 dark:bg-slate-950">
                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 text-center w-24">
                      Pos
                    </th>
                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
                      Service Description
                    </th>
                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 text-center w-32">
                      Qty
                    </th>
                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 text-right w-48">
                      Unit Price
                    </th>
                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.3em] text-white text-right w-48">
                      Total ({quotation.currency})
                    </th>
                  </tr>
                </thead>
                  ...
                  {(quotation as any).items?.map((item: any, index: number) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-10">
                        <p className="font-black text-slate-900 dark:text-white mb-2 text-lg tracking-tight group-hover:text-indigo-600 transition-colors uppercase">
                          {item.name}
                        </p>
                        {item.description && (
                          <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold line-clamp-2 max-w-xl leading-relaxed">
                            {item.description}
                          </p>
                        )}
                      </td>
                      <td className="px-8 py-10 text-sm font-black text-slate-900 dark:text-white text-center">
                        <div className="inline-flex flex-col items-center">
                          <span className="bg-slate-100 dark:bg-slate-800 px-4 py-1.5 rounded-xl font-black text-base">
                            {item.quantity}
                          </span>
                          <span className="text-[9px] text-slate-400 uppercase font-black mt-1.5 tracking-widest">
                            {item.unit || "pcs"}
                          </span>
                        </div>
                      </td>
              ...
              <div className="min-w-[400px] space-y-8">
                <div className="space-y-5 px-8 py-8 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-200/60 dark:border-slate-800">
                  ...
                  {Number(quotation.taxAmount) > 0 && (
                    <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.25em]">
                      ...
                      <span className="text-slate-900 dark:text-white tabular-nums text-sm">
                        +{quotation.currency}{" "}
                        {Number(quotation.taxAmount).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </div>
                  ...
                  {Number(quotation.discountAmount) > 0 && (
                    <div className="flex justify-between items-center text-[10px] font-black text-emerald-500 uppercase tracking-[0.25em]">
                      ...
                      <span className="tabular-nums text-sm">
                        -{quotation.currency}{" "}
                        {Number(quotation.discountAmount).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </div>
                ...
                <div className="bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 rounded-[3rem] p-10 text-white shadow-2xl shadow-indigo-500/40 relative overflow-hidden group">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-[11px] font-black uppercase tracking-[0.4em] text-indigo-100/70">
                      Final Total Balance
                    </span>
                  ...
                  <div className="flex items-baseline gap-4">
                    <span className="text-3xl font-black text-indigo-200/80">
                      {quotation.currency}
                    </span>
                    <span className="text-6xl font-black tracking-tighter tabular-nums drop-shadow-lg">
                      {Number(quotation.totalAmount).toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
            ...
            <div className="mt-32 pt-16 border-t border-slate-100 dark:border-slate-800 text-center relative">
              ...
              <p className="text-[11px] text-slate-400 font-black uppercase tracking-[0.4em] mb-4 leading-loose">
                Thank you for your business / {businessSettings?.company?.name || "our company"}
                <br />
