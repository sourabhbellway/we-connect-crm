...
const QuotationsPage: React.FC = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const mappedQuotations = items.map((q: any) => ({
          ...q,
          customerName: q.lead
            ? `${q.lead.firstName || ""} ${q.lead.lastName || ""}`.trim() || q.lead.company
            : q.deal?.title || "-",
  ...
  const handleDownloadPDF = async (quotationId: string) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || t("quotations.download_error") || "Failed to download PDF"
      );
    }
  };
  ...
  const handlePreviewPDF = async (quotationId: string) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || t("quotations.preview_error") || "Failed to preview PDF"
      );
    }
  };
  ...
  const handleDelete = async (quotationId: string) => {
    if (
      !window.confirm(
        t("quotations.confirm_delete") || "Are you sure you want to delete this quotation?"
      )
    ) {
      return;
    }
  ...
  const handleBookOrder = (quotationId: string, e?: React.MouseEvent) => {
    if (e) {
      e.stopPropagation();
    }
    navigate(`/invoices/new?quotationId=${quotationId}`);
  };
                  ...
                  >
                    <div className="p-5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-750">
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(quotation.status)}`}
                        >
                      ...
                      <div className="pt-2 flex items-center justify-between">
                        ...
                        {hasPermission("deal.update") && (
                          <button
                            ...
                            className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-md flex items-center gap-1"
                          >
                            <Plus size={12} />
                            Create Invoice
                          </button>
                        ...
                        {visibleColumns.includes("related") && (
                          <div className="col-span-1 min-w-0">
                            <div className="text-xs text-gray-600 dark:text-gray-400 truncate capitalize">
                              {quotation.relatedType
                                ? `${quotation.relatedType}${quotation.relatedTo ? `: ${quotation.relatedTo}` : ""}`
                                : "-"}
                            </div>
                          </div>
                        )}
