...
const CreateQuotationPage: React.FC = () => {
  ...
  const selectProduct = (index: number, product: any) => {
    ...
    const productName =
      product.name || product.title || product.itemName || product.productName || "";
  ...
  const handleSubmit = async (sendImmediately = false) => {
    ...
    try {
      ...
      const finalSubject =
        subject ||
        (relatedType === "lead" && toField ? `Quotation for ${toField}` : "New Quotation");
  ...
  const getProductDescription = (product: any) => {
    return (
      product.description || product.details || product.longDescription || product.summary || ""
    );
  };
            ...
            <Card padding="lg">
              <div className="grid grid-cols-12 gap-4">
                ...
                <div className="col-span-12" ref={searchRef}>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                    <span className="text-red-500">*</span> Lead
                  </label>
                  <div className="relative">
                    ...
                    >
                      ...
                      <ChevronDown
                        size={16}
                        className="text-gray-400 group-hover:text-blue-500 flex-shrink-0 ml-2 transition-colors"
                      />
                    </button>
                        ...
                        <div className="overflow-y-auto max-h-48">
                          {searchResults.length > 0 ? (
                            searchResults.map((entity) => {
                              ...
                              const displayName =
                                names.length > 0 ? names.join(" ") : entity.email || "Unknown Lead";
                              const initials =
                                names.length > 0
                                  ? names
                                      .map((n: string) => n[0])
                          ...
                          >
                            ...
                            <td className="px-4 py-3 relative">
                              <div
                                className="relative"
                                ref={
                                  activeProductDropdownIndex === index ? productDropdownRef : null
                                }
                              >
                                <button
                                  type="button"
                                  onClick={() =>
                                    setActiveProductDropdownIndex(
                                      activeProductDropdownIndex === index ? null : index
                                    )
                                  }
                                  className="w-full px-3 py-2 text-sm text-left border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white hover:border-blue-500 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all flex items-center justify-between group"
                                >
                                ...
                                {activeProductDropdownIndex === index && (
                                  <div
                                    ...
                                    onClick={() => setActiveProductDropdownIndex(null)}
                                  ></div>
                                )}
                            ...
                            <td className="px-4 py-3">
                              <textarea
                                ...
                                onChange={(e) =>
                                  updateItem(index, "longDescription", e.target.value)
                                }
                  ...
                  <div className="col-span-12">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (templateId) {
                          const template = termsAndConditions.find(
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          <option value="%">% Percentage</option>
                          <option value="fixed">
                            {currencySettings?.currencies?.find((c) => c.code === currency)
                              ?.symbol || currency}{" "}
                            Fixed
                          </option>
                        </select>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500 dark:text-gray-400">
                          {currencySettings?.currencies?.find((c) => c.code === currency)?.symbol ||
                            currency}
                        </span>
                      </div>
                    </div>
