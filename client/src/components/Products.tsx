          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-6">
            {isLoading ? (
              ...
              products.map((product) => (
                <div
                  key={product.id}
                  className={`p-4 rounded-lg border relative group ${selectedIds.includes(product.id) ? "ring-2 ring-indigo-500" : ""}`}
                >
                  <button
                    onClick={() => toggleSelection(product.id)}
                    className="absolute top-2 left-2 z-10 p-1 bg-white border rounded shadow-sm"
                  >
            ...
            <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900">
                <tr>
                  ...
                  <th
                    className={`px-6 py-3 text-start text-xs font-medium uppercase tracking-wider ${!isColumnVisible("product") ? "hidden" : ""} dark:text-white`}
                  >
                    Product
                  </th>
                  <th
                    className={`px-6 py-3 text-start text-xs font-medium uppercase tracking-wider ${!isColumnVisible("sku") ? "hidden" : ""} dark:text-white`}
                  >
                    SKU
                  </th>
                  <th
                    className={`px-6 py-3 text-start text-xs font-medium uppercase tracking-wider ${!isColumnVisible("category") ? "hidden" : ""} dark:text-white`}
                  >
                    Category
                  </th>
                  <th
                    className={`px-6 py-3 text-start text-xs font-medium uppercase tracking-wider ${!isColumnVisible("price") ? "hidden" : ""} dark:text-white`}
                  >
                    Price
                  </th>
                  <th
                    className={`px-6 py-3 text-start text-xs font-medium uppercase tracking-wider ${!isColumnVisible("status") ? "hidden" : ""} dark:text-white`}
                  >
                    Status
                  </th>
                  <th className="px-6 py-3 text-end text-xs font-medium uppercase tracking-wider dark:text-white">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  ...
                  products.map((p) => (
                    <tr key={p.id} className={selectedIds.includes(p.id) ? "bg-indigo-50/30" : ""}>
                      ...
                      <td className={`px-6 py-4 ${!isColumnVisible("product") ? "hidden" : ""}`}>
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500 dark:text-gray-300 truncate w-40">
                              {p.description}
                            </div>
                          </div>
                        </div>
                      ...
                      {/* --- FIXED: Dark Mode Text --- */}
                      <td
                        className={`px-6 py-4 text-sm ${!isColumnVisible("sku") ? "hidden" : ""} dark:text-white`}
                      >
                        {p.sku || "-"}
                      </td>
                      {/* --- FIXED: Dark Mode Text --- */}
                      <td
                        className={`px-6 py-4 text-sm ${!isColumnVisible("category") ? "hidden" : ""} dark:text-white`}
                      >
                        {p.category || "-"}
                      </td>
                      {/* --- FIXED: Dark Mode Text --- */}
                      <td
                        className={`px-6 py-4 text-sm ${!isColumnVisible("price") ? "hidden" : ""} dark:text-white`}
                      >
                        {formatCurrency(p.price, p.currency)}
                      </td>
                      <td className={`px-6 py-4 ${!isColumnVisible("status") ? "hidden" : ""}`}>
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${p.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                        >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-lg overflow-hidden shadow-xl border">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Upload CSV. Must have 'name' and 'price'.
              </p>
              <button
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 border rounded-lg text-gray-900 dark:text-white"
                >
                  Cancel
                </button>
                <button
