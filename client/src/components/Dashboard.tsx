...
function Dashboard() {
  ...
  const totalDeals =
    (dashboardKPIs?.revenue.wonDeals || 0) +
  ...
  const canViewActivityEngagement = hasPermission(
    PERMISSIONS.DASHBOARD_WIDGETS.ACTIVITY_ENGAGEMENT
  );
      ...
      try {
        if (hasPermission("lead.read")) {
          const response = await leadService.getDashboardKPIs(
            undefined,
            undefined,
            scope === "me" ? (user?.id as any) : undefined,
            scope
          );
    ...
    const fetchLeadStats = async () => {
      try {
        if (hasPermission("lead.read")) {
          ...
          if (scope === "me" && user?.id) {
            const res: any = await leadService.getLeads({
              page: 1,
              limit: 100,
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-slate-900 min-h-screen">
      <div className="header bg-hero-gradient bg-cover bg-center relative overflow-hidden">
            ...
            <div className="grid gap-10 xl:grid-cols-2">
              <div className="space-y-5 text-white">
                ...
                <p className="text-white/80 text-base sm:text-lg leading-relaxed max-w-2xl">
                  ...
                  <span className="text-white font-semibold">{leadStats.totalLeads} leads</span> and{" "}
                  <span className="text-white font-semibold">{totalDeals} total deals</span> in your{" "}
                  {scope === "all" ? "organization" : "pipeline"}.
                </p>
              </div>
              ...
              : hasActivities
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Jump straight into Task Management with this date prefilled. Ideal for
                    scheduling follow-ups, reminders, or any activity uncovered on the calendar.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-1 bg-gray-200 dark:bg-gray-800 rounded-2xl  hover:shadow-xl transition-all duration-300 p-6 sm:p-8 border border-gray-100 dark:border-gray-700 mt-6 lg:mt-0">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/10 dark:to-blue-800/20 rounded-lg">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Deals won vs lost</p>
                </div>
                ...
                <div className="p-4 bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/10 dark:to-green-800/20 rounded-lg">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    {isKPILoading
                      ? ""
                      : `${dashboardKPIs?.conversion.convertedLeads || 0} of ${dashboardKPIs?.conversion.totalLeads || 0} leads`}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <p className="text-xl font-bold text-purple-600 dark:text-purple-400">
                      {isKPILoading
                        ? "..."
                        : `${dashboardKPIs?.conversion.avgSalesCycleDays || 0}d`}
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    ...
                    <p className="text-xl font-bold text-orange-600 dark:text-orange-400">
                      {isKPILoading
                        ? "..."
                        : `${dashboardKPIs?.conversion.avgResponseTimeHours || 0}h`}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    icon: (
                      <HiOutlineCheck className="w-5 h-5 text-purple-600 dark:text-purple-400" />
                    ),
                  },
                  {
                    label: "Active Deals",
                    ...
                    icon: (
                      <HiOutlineClock className="w-5 h-5 text-orange-600 dark:text-orange-400" />
                    ),
                  },
                  {
                    label: "Lost Deals",
                      ...
                      {isKPILoading
                        ...
                        : kpi.isCurrency
                          ? new Intl.NumberFormat("en-US", {
                              style: "currency",
                              currency:
                                dashboardKPIs?.revenue?.currency?.code &&
                                ["USD", "EUR", "GBP", "INR", "JPY", "CAD", "AUD"].includes(
                                  dashboardKPIs.revenue.currency.code
                                )
                                  ? dashboardKPIs.revenue.currency.code
                                  : "USD",
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    color: "text-orange-600 dark:text-orange-400",
                    icon: (
                      <HiOutlineChartBar className="w-5 h-5 text-orange-600 dark:text-orange-400" />
                    ),
                  },
                ].map((kpi, idx) => (
                  <div key={idx} className={`p-4 rounded-lg ${kpi.bg} flex flex-col`}>
