...
import {
    ArrowLeft, ArrowRight, User as UserIcon, Mail, Phone, Building as BuildingIcon, Calendar, UserCheck,
    ...
    Trash2, Eye, Plus, PhoneCall, CreditCard, Package,
    Briefcase as BriefcaseIcon, Globe, MapPin, User, Building
} from 'lucide-react';
...
const LeadProfile: React.FC = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const productsData = productsResp?.data?.products || productsResp?.data || productsResp || [];
    ...
    const fetchCallLogs = async (leadId: number, showLoader = true) => {
        try {
            ...
            const newCallLogs = data.data?.items || data.data?.callLogs || data.callLogs || data.items || [];
        ...
        visibleFields.forEach(field => {
            ...
            if (value !== undefined && value !== null && value !== '') {
                ...
                if (field.fieldName === 'budget' || field.fieldName === 'annualRevenue') {
                    ...
                    displayValue = product ? `${product.name} ${product.sku ? `(${product.sku})` : ''}` : value;
    ...
    const updateCallLogDuration = async (callLogId: number, duration: number) => {
        try {
            ...
            setCallLogs(prevLogs => prevLogs.map(log =>
                log.id === callLogId
                    ? { ...log, callStatus: 'COMPLETED', duration: duration, endTime: new Date().toISOString() }
                    : log
            ));
        ...
        try {
            ...
            const response = await fetch('/api/call-logs', {
                ...
                body: JSON.stringify({
                    ...
                    startTime: startTime,
                    endTime: new Date(new Date(startTime).getTime() + (parseInt(callLogForm.duration || '0') * 60000)).toISOString()
                }),
            });
                            ...
                            <div className="space-y-6">
                                {fieldConfigsLoading ? (
                                    ...
                                    getGroupedFields().map((section: any) => (
                                        <div key={section.key} className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                                            <div className="flex items-center mb-6">
                                                <div className={`p-2 rounded-lg bg-${section.color}-50 dark:bg-${section.color}-900/20 text-${section.color}-600 dark:text-${section.color}-400 mr-3`}>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                                {section.fields.map((field: any) => (
                                                    <div key={field.fieldName}>
                                                        ...
                                                        {field.fieldName === 'website' || field.fieldName === 'linkedinProfile' ? (
                                                            <a
                                    ...
                                    <div className="bg-white dark:bg-gray-800 rounded-lg p-12 text-center border border-gray-200 dark:border-gray-700">
                                        ...
                                        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No Details Available</h3>
                                            ...
                                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-center">Quantity</th>
                                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-right">Price</th>
                                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider text-right">Total</th>
                                                    </tr>
                                                </thead>
                                                ...
                                                <tfoot>
                                                    <tr className="bg-gray-50 dark:bg-gray-900/50">
                                                        ...
                                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-blue-600 dark:text-blue-400 text-right">
                                                            {formatCurrency(
                                                                (lead.products as any[]).reduce((sum, p) => sum + (p.price * p.quantity), 0),
                                                                lead.currency || 'USD'
                                                            )}
                                                        </td>
                                                    </tr>
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
                                ...
                                <div className="space-y-4">
                                    {emailsLoading ? (
                                        ...
                                        <div className="space-y-4">
                                            {emails.map((email) => (
                                                <div key={email.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                                                    <div className="flex items-center justify-between mb-3">
                                                        <div className="flex items-center space-x-3">
                                                            <Mail className={`h-5 w-5 ${email.direction === 'INBOUND' ? 'text-blue-500' : 'text-green-500'}`} />
                                                            <div>
                                                                <h4 className="font-medium text-gray-900 dark:text-white">{email.subject || 'No Subject'}</h4>
                                                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                                                    {new Date(email.sentAt || email.createdAt).toLocaleString()}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${email.direction === 'INBOUND' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                                                            {email.direction}
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="space-y-2">
                                                        {email.direction === 'INBOUND' && email.user ? (
                                                            <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/10 p-2 rounded border-blue-100 dark:border-blue-800">
                                                                <span className="font-medium text-gray-700 dark:text-gray-300 mr-2">From:</span>
                                                                <span className="font-medium text-gray-900 dark:text-white">{email.user.firstName} {email.user.lastName} &lt;{email.user.email}&gt;</span>
                                                            </div>
                                                        ) : (
                                                            <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                                                                <span className="font-medium text-gray-700 dark:text-gray-300 mr-2">To:</span>
                                                        ...
                                                        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600 text-sm text-gray-900 dark:text-white">
                                                            <p className="whitespace-pre-wrap">{email.content || 'No content preview available.'}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
