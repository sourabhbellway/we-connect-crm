...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantStyles: Record<keyof typeof BUTTON_VARIANTS, string> = {
      ...
      SECONDARY:
        "bg-white dark:bg-slate-800 text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-slate-700 focus:ring-gray-300 shadow border border-gray-200 dark:border-slate-600",
      OUTLINE:
        "border border-weconnect-red text-weconnect-red hover:bg-weconnect-red hover:text-white focus:ring-red-400 shadow-sm",
      GHOST:
        "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 focus:ring-gray-300",
      DESTRUCTIVE:
        "bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700 focus:ring-red-400 shadow-lg hover:shadow-xl",
    };
    ...
    const iconElement = loading ? <Loader2 className={`${iconSizes[size]} animate-spin`} /> : icon;
