...
import { FileText, Phone, Calendar, User, RotateCcw, Trash2 } from "lucide-react";
...
const TrashLeads: React.FC = () => {
  ...
  const handlePermanentDelete = async (leadId: number) => {
    ...
    try {
      await leadService.deleteLeadPermanently(leadId);
      toast.success("Lead deleted permanently!");
      fetchData(page);
    } catch (e: any) {
      const message = e?.response?.data?.message || "Failed to permanently delete lead";
      toast.error(message, { toastId: "trash_leads_permanent_delete_error" });
    }
  };
