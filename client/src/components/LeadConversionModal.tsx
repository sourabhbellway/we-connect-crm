...
import { X, DollarSign, CheckCircle, AlertCircle, RefreshCw } from "lucide-react";
...
const LeadConversionModal: React.FC<LeadConversionModalProps> = ({
  ...
  const [conversionData, setConversionData] = useState<ConversionData>({
    ...
    companyData: {},
    dealData: {
      currency: getEffectiveDefaultCurrency(),
      status: "",
      probability: 50,
    },
  });
    ...
    if (lead) {
      setConversionData((prev) => ({
        ...
        dealData: {
          ...
          currency:
            getEffectiveDefaultCurrency() || (lead as any)?.currency || prev.dealData.currency,
          status:
            prev.dealData.status ||
            (dealStatuses?.length > 0
              ? dealStatuses.find((s) => s.isActive)?.name || dealStatuses[0].name
              : "DRAFT"),
        },
      }));
    }
  }, [lead, dealStatuses, currencySettings]);

  // Separate effect to ensure currency syncs if settings load late
  useEffect(() => {
    const defaultCurrency = getEffectiveDefaultCurrency();
    if (defaultCurrency) {
      setConversionData((prev) => {
        // If we have a default and the current one is either empty or matches a hardcoded "USD"
        // which might be a stale default, we update it to the true default.
        if (!prev.dealData.currency || prev.dealData.currency === "USD") {
          return {
            ...prev,
            dealData: {
              ...prev.dealData,
  ...
  const handleInputChange = (section: keyof ConversionData, field: string, value: any) => {
    ...
    try {
      ...
      const cleanedData = {
        ...
        contactData: Object.fromEntries(
          Object.entries(conversionData.contactData).filter(
            ([_, value]) => value !== "" && value !== null && value !== undefined
          )
        ),
        companyData: Object.fromEntries(
          Object.entries(conversionData.companyData).filter(
            ([_, value]) => value !== "" && value !== null && value !== undefined
          )
        ),
        dealData: Object.fromEntries(
          Object.entries(conversionData.dealData).filter(
            ([_, value]) => value !== "" && value !== null && value !== undefined
          )
        ),
      };
      ...
      <div className="relative mx-auto p-5 border max-w-4xl w-full shadow-lg rounded-md bg-white dark:bg-gray-800">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            {/* Create Deal */}
            <div
              className={`border rounded-lg p-4 ${conversionData.createDeal ? "border-purple-500 bg-purple-50 dark:bg-purple-900/20" : "border-gray-200 dark:border-gray-700"}`}
            >
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("dealData", "value", parseFloat(e.target.value) || 0)
                        }
                        min="0"
                        step="0.01"
                        disabled={isConverting}
                    ...
                    <div>
                      ...
                      <select
                        value={conversionData.dealData.currency || getEffectiveDefaultCurrency()}
                      ...
                      >
                        {currencySettings?.currencies?.length ? (
                          [...currencySettings.currencies]
                            .sort((a, b) => {
                              const def = getEffectiveDefaultCurrency();
                              if (a.code === def) return -1;
                              if (b.code === def) return 1;
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("dealData", "probability", parseInt(e.target.value) || 0)
                      }
                      min="0"
                      max="100"
                      disabled={isConverting}
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm">
                ...
                <ul className="text-blue-700 dark:text-blue-300 mt-1 space-y-1">
                  ...
                  {conversionData.createDeal && (
                    <li>• A new deal/opportunity will be created and linked</li>
                  )}
                </ul>
              </div>
            </div>
