...
const DealEdit: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        setLoading(true);
        const res = await dealService.getDealById(Number(id));
        const d = res.data as any as Deal | any;
        setForm({
          title: d.title || "",
          value: String(d.value ?? ""),
          currency: d.currency || currencySettings?.primary || "USD",
        ...
        toast.error(e?.response?.data?.message || "Failed to load deal");
        navigate("/deals");
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dealService.updateDeal(Number(id), payload);
