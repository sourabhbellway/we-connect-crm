    ...
    const run = async () => {
      ...
      try {
        const [lr, dr] = await Promise.all([
          leadService
            .getLeads({ page: 1, limit: 5, search: term })
            .catch(() => ({ data: { leads: [] } })),
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && debounced.trim().length >= 2 && (
        <div className="absolute z-50 mt-2 w-full max-w-xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="px-3 py-2 text-[11px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-slate-800">
            Press Enter to view full results
          </div>
        </div>
      )}
    </div>
