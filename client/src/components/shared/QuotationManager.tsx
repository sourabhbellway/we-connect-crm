  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{labels.title}</h2>
        {hasPermission(`${entityType}.create`) && (
          <div className="flex items-center space-x-2">
            <Button
              ...
              onClick={() =>
                navigate(`/quotations/new?entityType=${entityType}&entityId=${entityId}`)
              }
            >
              <Plus size={16} />
        ...
        <div className="space-y-4">
          {quotations.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">{labels.emptyQuotationDesc}</p>
              {hasPermission(`${entityType}.create`) && (
                <Button
                  onClick={() =>
                    navigate(`/quotations/new?entityType=${entityType}&entityId=${entityId}`)
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">{quotation.title}</p>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        onClick={async () => {
                          try {
                            const res = await apiClient.get(
                              `/quotations/${quotation.id}/pdf/preview`,
                              { responseType: "blob" }
                            );
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            const res = await apiClient.get(
                              `/quotations/${quotation.id}/pdf/download`,
                              { responseType: "blob" }
                            );
        ...
        <div className="space-y-4">
          {invoices.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">{labels.emptyInvoiceDesc}</p>
              {hasPermission(`${entityType}.create`) && (
                <Button
                  onClick={() =>
                    navigate(`/invoices/new?entityType=${entityType}&entityId=${entityId}`)
                    ...
                    <div className="flex items-center space-x-2">
                      <Button
                        ...
                        onClick={async (e) => {
                          e.stopPropagation();
                          try {
                            const res = await apiClient.get(`/invoices/${invoice.id}/pdf/preview`, {
                              responseType: "blob",
                            });
                            const url = URL.createObjectURL(
                              new Blob([res.data], { type: "application/pdf" })
