  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Payment Tracker</h2>
            ...
            <div className="space-y-4">
              {filteredInvoices.map((invoice) => (
                <Card key={invoice.id} className="p-4 hover:shadow-lg transition-shadow">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div>{getStatusIcon(invoice.status, "invoice")}</div>
                      <div>
                        ...
                        <div className="flex items-center space-x-4 text-xs text-gray-500 dark:text-gray-400 mt-1">
                          ...
                          {invoice.dueDate && (
                            <span
                              className={
                                isOverdue(invoice.dueDate) ? "text-red-600 font-medium" : ""
                              }
                            >
                    ...
                    <div className="text-right">
                      <div className="flex items-center space-x-3 mb-2">
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(invoice.status, "invoice")}`}
                        >
                      ...
                      {invoice.status === "PARTIALLY_PAID" && (
                        <div className="text-sm">
                          ...
                          <span className="text-orange-600 ml-2">
                            Due: {formatCurrency(invoice.dueAmount, invoice.currency)}
                          </span>
                        </div>
                      )}
                  ...
                  <Card key={payment.id} className="p-4 hover:shadow-lg transition-shadow">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="flex items-center space-x-3 mb-2">
                          <span
                            className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(payment.status, "payment")}`}
                          >
            ...
            {invoices.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  You need to create an invoice before you can record a payment. Payments must be
              ...
              <>
                    ...
                    >
                      ...
                      {invoices
                        .filter(
                          (inv) =>
                            (inv.dueAmount ??
                              Math.max(inv.totalAmount - (inv.paidAmount || 0), 0)) > 0
                        )
                        .map((inv) => (
                          <option key={String(inv.id)} value={String(inv.id)}>
                            {inv.invoiceNumber} • Due{" "}
                            {formatCurrency(
                              inv.dueAmount ?? Math.max(inv.totalAmount - (inv.paidAmount || 0), 0),
                              inv.currency
                            )}
                          </option>
                        ))}
                ...
                <div className="mt-6 flex items-center gap-3 justify-end">
                  ...
                  <Button
                          ...
                          setPayments((prev) => [
                            {
                              ...
                              paymentNumber:
                                created.paymentNumber || `PMT-${created.id || Date.now()}`,
                    ...
                    className="bg-weconnect-red hover:bg-red-600 text-white"
                  >
