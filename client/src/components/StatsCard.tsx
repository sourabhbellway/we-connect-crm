...
function StatsCard({
  ...
  return (
    <div
      className={`${colors.bg} rounded-xl p-4 shadow-sm  hover:shadow-md transition-all duration-200 ${className}`}
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-medium text-white/70 dark:text-gray-300 mb-1">{title}</h3>
          ...
          {subtitle && <p className="text-xs text-white/70 dark:text-gray-400 mb-2">{subtitle}</p>}

          {trend && (
            <div className={`flex items-center space-x-1 ${getTrendColor()}`}>
              {getTrendIcon()}
              <span className="text-sm font-medium">{Math.abs(trend.value)}%</span>
              <span className="text-xs text-white/70 dark:text-gray-400">{trend.label}</span>
            </div>
          )}
        </div>
      </div>
