...
const LeadFieldsSettings: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fetchFields();
      try {
        window.dispatchEvent(new CustomEvent("fieldConfigsUpdated"));
      } catch {}
    } catch (error) {
  ...
  const handleToggleVisibility = async (field: FieldConfig) => {
    try {
      ...
      setFields(fields.map((f) => (f.id === field.id ? { ...f, isVisible: !f.isVisible } : f)));
  ...
  const handleDeleteSection = async (id: number) => {
    if (
      !confirm(
        "Delete this section? This will not delete fields in it, but they will become unassigned."
      )
    )
      return;
    try {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            Manage Lead Form Fields
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Show, hide, reorder, and configure lead form fields
          </p>
        </div>
        <button
          ...
          <div className="flex flex-wrap gap-2">
            {sections.map((s) => (
              ...
              >
                ...
                {!["personal", "company", "location", "lead_management", "notes"].includes(
                  s.key
                ) && (
                  <button
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span
                    ...
                    ${
                      getSectionColor(field.section) === "blue"
                        ? "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
                        : getSectionColor(field.section) === "green"
                          ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300"
                          : getSectionColor(field.section) === "purple"
                            ? "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300"
                            : getSectionColor(field.section) === "orange"
                              ? "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300"
                              : getSectionColor(field.section) === "indigo"
                                ? "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300"
                                : getSectionColor(field.section) === "red"
                                  ? "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300"
                                  : "bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300"
                    }`}
                  >
                    {sections.find((s) => s.key === field.section)?.label ||
      ...
      <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700">
        <p className="text-xs text-gray-600 dark:text-gray-400">
          Total fields: {fields.length} • Visible: {fields.filter((f) => f.isVisible).length} •
