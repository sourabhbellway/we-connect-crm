...
const getStatusStyles = (statusName: string, dealStatuses: DealStatus[]) => {
  const status = dealStatuses.find(
    (s) => s.name.toUpperCase() === (statusName || "").toUpperCase()
  );
...
const Deals: React.FC = () => {
  ...
  const handleDownloadTemplate = () => {
    ...
    const rows = [
      ["Sample Deal", "Sample Description", "1000", "USD", "DRAFT", "50", "2023-12-31"],
    ];
  ...
  const handleBulkAssign = async (userId: string) => {
    ...
    try {
      ...
      setDeals((prev) =>
        prev.map((d) =>
          selectedIds.includes(d.id) ? { ...d, assignedTo: targetUserId ?? undefined } : d
        )
      );
      setSelectedIds([]);
          ...
          <div className="w-full sm:w-48 sm:min-w-[220px]">
            <DropdownFilter
              ...
              options={[
                ...
                ...(dealStatuses || []).map((s) => ({ value: s.name, label: s.name })),
              ]}
            />
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${getPriorityColor(priority)}`}
                      >
                    ...
                    <div className="px-4 pb-3 flex-1">
                      <h3
                        ...
                        onClick={() =>
                          hasPermission(PERMISSIONS.DEAL.READ) && navigate(`/deals/${deal.id}`)
                        }
                      >
                        {deal.title}
                      </h3>
                          ...
                          {deal.assignedUser ? (
                            <div className="h-6 w-6 rounded-full bg-gradient-to-br from-[#EF444E] to-[#ff5a64] flex items-center justify-center">
                              <span className="text-white text-xs font-medium">
                                {deal.assignedUser.firstName?.[0] || ""}
                                {deal.assignedUser.lastName?.[0] || ""}
                              </span>
                            </div>
                          ) : (
                  ...
                  <DropdownFilter
                    ...
                    onChange={async (v) => {
                      ...
                      try {
                        setDeals((prev) =>
                          prev.map((d) =>
                            selectedIds.includes(d.id) ? { ...d, status: target as any } : d
                          )
                        );
                        await Promise.all(
                          selectedIds.map((id) =>
                            dealService.updateDeal(id, { status: target as any })
                          )
                        );
                    ...
                    <tr>
                      <th className="px-4 py-3">
                        <input
                          ...
                          checked={
                            pageItems.length > 0 &&
                            pageItems.every((d) => selectedIds.includes(d.id))
                          }
                          onChange={(e) => {
                      ...
                      <th
                        className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible("title") ? "hidden" : ""}`}
                      >
                        <TableSortHeader
                          label="Title"
                          column={"title"}
                      ...
                      <th
                        className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible("value") ? "hidden" : ""}`}
                      >
                        <TableSortHeader
                          label="Value"
                          column={"value"}
                      ...
                      <th
                        className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible("contact") ? "hidden" : ""}`}
                      >
                        <TableSortHeader
                          label="Contact"
                          column={"contact"}
                      ...
                      <th
                        className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible("expectedCloseDate") ? "hidden" : ""}`}
                      >
                        <TableSortHeader
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {pageItems.map((deal) => (
                      ...
                      >
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap ${!isColumnVisible("title") ? "hidden" : ""}`}
                          data-label="Title"
                        >
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap ${!isColumnVisible("value") ? "hidden" : ""}`}
                          data-label="Value"
                        >
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap ${!isColumnVisible("contact") ? "hidden" : ""}`}
                          data-label="Contact"
                        >
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-sm ${!isColumnVisible("status") ? "hidden" : ""}`}
                          data-label="Status"
                        >
                          {hasPermission(PERMISSIONS.DEAL.UPDATE) ? (
                            <select
                              ...
                              value={deal.status}
                              onChange={(e) =>
                                updateStatusInline(deal.id, e.target.value as Deal["status"])
                              }
                            >
                              {(dealStatuses || [])
                                .filter((s) => s.isActive)
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 ${!isColumnVisible("expectedCloseDate") ? "hidden" : ""}`}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                <p>
                  Upload a CSV file with your deals data. Make sure it contains at least{" "}
                  <strong>title</strong> and <strong>value</strong> columns.
                </p>
                <button
                  onClick={handleDownloadTemplate}
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  <label htmlFor="deal-import-input" className="cursor-pointer group">
                    <Upload
                      className={`mx-auto h-12 w-12 mb-3 transition-colors ${importFile ? "text-green-500" : "text-gray-400 group-hover:text-blue-500"}`}
                    />
