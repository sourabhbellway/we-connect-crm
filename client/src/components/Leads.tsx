...
const Leads: React.FC = () => {
    ...
    const handleStatusChange = async (leadId: number, newStatus: string) => {
        ...
        try {
            ...
            if (shouldConvert) {
                ...
                toast.success(t("leads.statusUpdateSuccess", "Lead status updated successfully"));
            }

            await refreshLeadsCount();
            await refreshDealsCount();
        } catch (err: any) {
            const message = err?.response?.data?.message || t("leads.statusUpdateError", "Failed to update status");
            toast.error(message);
        } finally {
        ...
        try {
            ...
            if (alreadyConverted && leadToConvert) {
                try {
                    ...
                    toast.success(t("leads.convertSuccess", "Lead converted successfully!"));
                    setShowConversionModal(false);
                    setLeadToConvert(null);
                    fetchLeads();
                    await Promise.all([
    ...
    const noResultsDescription = isSearchActive || isStatusActive
        ? t("leads.noResultsSearchDescription", "No leads match your search and filters. Try adjusting your filters or search terms. You can also clear all filters to see all leads.")
        : t("leads.noResultsDescription", "No leads found.");
    ...
    return (
        <div className="space-y-6 p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
            ...
            <div className="space-y-4">
                <ListToolbar
                    title={t("leads.title")}
                    subtitle={t("leads.subtitle") || "Capture and manage your incoming opportunities"}
                    addLabel={t("leads.addLead")}
                    onAdd={handleCreateLead}
                    actions={[
                        { label: t("leads.syncIntegrations") || 'Sync Integrations', onClick: handleSyncIntegrations, icon: <RefreshCw className="w-4 h-4" />, loading: isSyncingIntegrations },
                    ]}
                    bulkActions={[
                        { label: t("common.downloadTemplate") || 'Download Template', onClick: handleDownloadTemplate, icon: <FileDown className="w-4 h-4" /> },
                        { label: t("leads.importLeads") || 'Import Leads', onClick: () => setShowImportModal(true), icon: <Upload className="w-4 h-4" /> },
                        { label: t("leads.exportLeads") || 'Export Leads', onClick: handleExportLeads, icon: <FileDown className="w-4 h-4" />, disabled: isExporting },
                        ...(selectedLeadIds.size > 0 ? [
                            { label: `${t("leads.bulkAssign") || 'Bulk Assign'} (${selectedLeadIds.size})`, onClick: () => setShowBulkAssignModal(true), icon: <User className="w-4 h-4" /> }
                        ] : [])
                    ]}
                />
                ...
                <div className="flex flex-col sm:flex-row sm:items-end gap-4">
                    ...
                    <div className="">
                        ...
                        <SearchInput
                            ...
                            placeholder={t("leads.searchEmail") || "Search by email..."}
                            className="max-w-full"
                        />
                    </div>
                    ...
                    <div className="w-full sm:w-48 sm:min-w-[220px]">
                        <DropdownFilter
                            ...
                            placeholder={t("leads.allStatuses") || "All Statuses"}
                            options={[
                    ...
                    <div className="w-full sm:w-48 sm:min-w-[220px]">
                        <DropdownFilter
                            ...
                            placeholder={t("users.allUsers") || "All Users"}
                            options={[
                                ...users.map((u) => ({
                                    value: u.id.toString(),
                ...
                <div className="hidden sm:flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1 ms-auto">
                    <button
            ...
            {viewMode === 'card' ? (
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-100 dark:border-gray-700 transition-all duration-300 p-6">
                    {leads.length === 0 ? (
                        <NoResults
                            title={error ? t("common.networkError") : t("leads.noResults")}
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-500 dark:text-gray-400">
                                                        {getLeadCompany(lead) || 'No company'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="p-6">
                        <div className="mb-4">
                            <MetaBar
                                ...
                                columnConfig={{
                                    columns: [
                                        { id: 'name', label: t('leads.table.lead') },
                                        { id: 'email', label: t('leads.table.email') },
                                        { id: 'phone', label: t('leads.table.phone') },
                                        { id: 'company', label: t('leads.table.company') },
                                        { id: 'status', label: t('leads.table.status') },
                                        { id: 'assignedTo', label: t('leads.table.assignedTo') },
                                        { id: 'createdAt', label: t('leads.table.created') },
                                    ],
                                    visibleColumns,
                                    ...
                                    <thead className="bg-gray-50 dark:bg-gray-900">
                                        <tr>
                                            ...
                                            <th className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible('email') ? 'hidden' : ''}`}>
                                                <TableSortHeader label={t("leads.table.email") as string} column={'email'} sortBy={sortBy} sortOrder={sortOrder} onChange={(c: any) => onHeaderSort(c)} />
                                            </th>
                                            <th className={`px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${!isColumnVisible('phone') ? 'hidden' : ''}`}>
                                                <TableSortHeader label={t("leads.table.phone") as string} column={'phone'} sortBy={sortBy} sortOrder={sortOrder} onChange={(c: any) => onHeaderSort(c)} />
                                            </th>
                                                                ...
                                                                <div className="flex-shrink-0 h-10 w-10">
                                                                    <div className="h-10 w-10 rounded-full bg-gradient-to-br from-[#EF444E] to-[#ff5a64] flex items-center justify-center">
                                                                        <span className="text-white font-medium text-sm leading-none">
                                                                            {getLocalizedValue(lead.firstNameAr, lead.firstName)?.[0] || ''}
                                                                            {getLocalizedValue(lead.lastNameAr, lead.lastName)?.[0] || ''}
                                                                        </span>
                                                                    </div>
                                                                </div>
                        ...
                        <div className="mt-3">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                                    {t("leads.importModal.title") || "Import Leads from CSV"}
                                </h3>
                                <button
                                    onClick={() => {
                                        if (!isImporting) {
                            ...
                            <div className="space-y-4">
                                <div className="text-sm text-gray-600 dark:text-gray-400">
                                    <p className="mb-2">{t("leads.importModal.description") || "Upload a CSV file to import multiple leads at once."}</p>
                                    <ul className="list-disc list-inside space-y-1 text-xs">
                                        <li>{t("leads.importModal.downloadTemplate") || "Use our template for best results"}</li>
                                        <li>{t("leads.importModal.requiredColumns") || "Required columns: firstName, lastName, email"}</li>
                                        <li>{t("leads.importModal.maxSize") || "Maximum file size: 10MB"}</li>
                                        <li>{t("leads.importModal.skipDuplicate") || "Duplicate emails will be skipped"}</li>
                                    </ul>
                                </div>

                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        {t("leads.importModal.selectFile") || "Select CSV File"}
                                    </label>
                                    <input
                                        type="file"
                                        accept=".csv"
                                    ...
                                    {importFile && (
                                        <p className="mt-2 text-sm text-green-600 dark:text-green-400">
                                            {t("common.selected") || "Selected"}: {importFile.name}
                                        </p>
                                    )}
                                </div>
                                    ...
                                    >
                                        {isImporting ? (
                                            <>
                                                ...
                                                {t("leads.importModal.importing") || "Importing..."}
                                            </>
                                        ) : (
                                            <>
                                                <Upload className="h-4 w-4 mr-2" />
                                                {t("leads.importModal.import") || "Import Leads"}
                                            </>
                                        )}
                                    </button>
                                </div>
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                    <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
                        <div className="mt-3">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                                    {t("leads.bulkAssignModal.title") || "Bulk Assign Leads"}
                                </h3>
                                <button
                                    onClick={() => {
                                        if (!isBulkAssigning) {
                            ...
                            <div className="space-y-4">
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                    {t("leads.bulkAssignModal.description", { count: selectedLeadIds.size }) || `Assign ${selectedLeadIds.size} selected lead(s) to a user`}
                                </p>

                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        {t("leads.bulkAssignModal.assignToUser") || "Assign to User"}
                                    </label>
                                    <select
                                        ...
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white disabled:opacity-50"
                                    >
                                        <option value="">{t("leads.bulkAssignModal.selectUser") || "-- Select a user --"}</option>
                                        <option value="unassigned">{t("leads.bulkAssignModal.unassigned") || "Unassigned"}</option>
                                        {users.map(user => (
                                    ...
                                    >
                                        {isBulkAssigning ? (
                                            ...
                                            <>
                                                ...
                                                {t("leads.bulkAssign") || "Assign Leads"}
                                            </>
                                        )}
                                    </button>
                                </div>
