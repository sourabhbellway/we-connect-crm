...
class BusinessSettingsService {
  ...
  async getLeadSources(): Promise<BusinessSettingsResponse<LeadSource[]>> {
    return apiRequest.get(`${this.baseUrl}/lead-sources`);
  }

  async createLeadSource(data: Partial<LeadSource>): Promise<BusinessSettingsResponse<LeadSource>> {
    return apiRequest.post(`${this.baseUrl}/lead-sources`, data);
  }

  async updateLeadSource(id: string, data: Partial<LeadSource>): Promise<BusinessSettingsResponse<LeadSource>> {
    return apiRequest.put(`${this.baseUrl}/lead-sources/${id}`, data);
  }

  async deleteLeadSource(id: string): Promise<BusinessSettingsResponse<void>> {
    return apiRequest.delete(`${this.baseUrl}/lead-sources/${id}`);
  }

  async reorderLeadSources(sourceIds: string[]): Promise<BusinessSettingsResponse<LeadSource[]>> {
    return apiRequest.put(`${this.baseUrl}/lead-sources/reorder`, { sourceIds });
  }

  // Pipelines & Deal Stages
  async getPipelines(): Promise<BusinessSettingsResponse<Pipeline[]>> {
    return apiRequest.get(`${this.baseUrl}/pipelines`);
  }

  async getPipeline(id: string): Promise<BusinessSettingsResponse<Pipeline>> {
    return apiRequest.get(`${this.baseUrl}/pipelines/${id}`);
  }

  async createPipeline(data: Omit<Pipeline, 'id'>): Promise<BusinessSettingsResponse<Pipeline>> {
    return apiRequest.post(`${this.baseUrl}/pipelines`, data);
  }

  async updatePipeline(id: string, data: Partial<Pipeline>): Promise<BusinessSettingsResponse<Pipeline>> {
    return apiRequest.put(`${this.baseUrl}/pipelines/${id}`, data);
  }

  async deletePipeline(id: string): Promise<BusinessSettingsResponse<void>> {
    return apiRequest.delete(`${this.baseUrl}/pipelines/${id}`);
  }

  async setDefaultPipeline(id: string): Promise<BusinessSettingsResponse<Pipeline>> {
    return apiRequest.put(`${this.baseUrl}/pipelines/${id}/set-default`);
  }

  async getDealStages(pipelineId?: string): Promise<BusinessSettingsResponse<DealStage[]>> {
    const url = pipelineId 
      ? `${this.baseUrl}/deal-stages?pipelineId=${pipelineId}`
      : `${this.baseUrl}/deal-stages`;
    return apiRequest.get(url);
  }

  async createDealStage(data: Omit<DealStage, 'id'>): Promise<BusinessSettingsResponse<DealStage>> {
    return apiRequest.post(`${this.baseUrl}/deal-stages`, data);
  }

  async updateDealStage(id: string, data: Partial<DealStage>): Promise<BusinessSettingsResponse<DealStage>> {
    return apiRequest.put(`${this.baseUrl}/deal-stages/${id}`, data);
  }

  async deleteDealStage(id: string): Promise<BusinessSettingsResponse<void>> {
    return apiRequest.delete(`${this.baseUrl}/deal-stages/${id}`);
  }

  async reorderDealStages(stageIds: string[]): Promise<BusinessSettingsResponse<DealStage[]>> {
    return apiRequest.put(`${this.baseUrl}/deal-stages/reorder`, { stageIds });
  }
